...
def get_nakshatra_boundaries(jd_ut: float) -> Tuple[float, float]:
    """Given a jd_ut, finds the exact start and end jd of the active nakshatra."""
    current_nak = compute_nakshatra(jd_ut)
    n_idx = current_nak["nakshatra_index"]
    frac = current_nak["fraction"]
    ...
    for _ in range(15):
        mid = (low + high) / 2
        if compute_nakshatra(mid)["nakshatra_index"] == n_idx:
            high = mid
        else:
            low = mid
    start_jd = high
