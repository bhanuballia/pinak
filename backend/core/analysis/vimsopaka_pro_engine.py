...
def run_vimsopaka_assessment(vargas: Dict[str, Any], chart: Dict[str, Any], dasha: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for p in benefics:
        ...
        if 7.0 <= score <= 14.0 and p in GEMSTONE_MAP:
            ...
            top_gemstones.append({
                "planet": p,
                "stone": stone_name,
                "reason": f"Benefic planet for {lagna} Lagna needing activation/strengthening.",
                "score": round(score, 2)
            })
            
    # Fallback/Additional Suggestions to ensure at least 2 gemstones
    if len(top_gemstones) < 2:
        for p in benefics:
            if any(item["planet"] == p for item in top_gemstones):
                continue
            if p in GEMSTONE_MAP:
                score = final_ranking_scores.get(p, 10.0)
                stone_info = GEMSTONE_MAP[p]
                stone_name = stone_info.get("gemstone", "") if isinstance(stone_info, dict) else str(stone_info)
                top_gemstones.append({
                    "planet": p,
                    "stone": stone_name,
                    "reason": f"Auspicious gemstone for {lagna} Lagna to enhance planetary vibration.",
                    "score": round(score, 2)
                })
    ...
    summary = {
        ...
        "top_gemstone": top_gemstones[0] if top_gemstones else None,
        "top_gemstones": top_gemstones[:3]
    }
