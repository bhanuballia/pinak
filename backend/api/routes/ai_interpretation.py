...
def ai_section_insight(payload: Dict[str, Any] = Body(...)):
    try:
        ...
        section_title = payload.get("section_title", "Detailed Section")
        
        # We extract standard context, but the prompt will emphasize the section
        chart_context = extract_chart_context(chart_data)
        
        if section_id == "chart_analysis":
            system_instruction = (
                f"You are a master Vedic Astrologer. You are performing a highly detailed reading specifically for the "
                f"'{section_title}' section of a native's Kundali Report.\n\n"
                f"Analyze the native's chart context exclusively through the lens of '{section_title}'. "
                f"Break down the life predictions into distinct topics, ensuring you explicitly cover: "
                f"Emotion, Luck, Career/Profession (specifically explaining which career paths are best), "
                f"Marriage, Study/Education (specifically explaining which fields of study will help the most), "
                f"and Personal Life, along with other major life areas (such as Wealth, Health, and Spirituality). "
                f"Explain each topic in exactly 100 words based on the native's chart.\n\n"
                f"Keep the response highly structured using Markdown formatting (headings for each topic, bullet points, bold text).\n"
                f"CRITICAL INSTRUCTION: You must complete your response fully. Do not leave any sentences incomplete or truncated at the end."
            )
        else:
            system_instruction = (
                f"You are a master Vedic Astrologer. You are performing a highly detailed reading specifically for the "
                f"'{section_title}' section of a native's Kundali Report.\n\n"
                f"Analyze the native's chart context exclusively through the lens of '{section_title}'. "
                f"Extract and emphasize any planetary placements, dignities, strengths, or dashas that are most relevant to this specific topic. "
                f"Provide actionable insights, deep esoteric meaning, and if applicable, remedial measures.\n\n"
                f"Keep the response highly structured using Markdown formatting (headings, bullet points, bold text).\n"
                f"CRITICAL INSTRUCTION: Keep the response beautifully written but concise, ideally under 400 words total to prevent truncation."
            )
        
        prompt = (
            f"{system_instruction}\n\n"
            f"Here is the native's chart context:\n\n{chart_context}\n\n"
            f"Please generate a profound '{section_title}' reading."
        )
        ...
        import traceback
        error_trace = traceback.format_exc()
        print(f"[AI ERROR] Section insight reading failed: {error_trace}")
        err_msg = str(e)
        if "Quota exceeded" in err_msg or "429" in err_msg or "ResourceExhausted" in err_msg:
            raise HTTPException(
                status_code=429,
