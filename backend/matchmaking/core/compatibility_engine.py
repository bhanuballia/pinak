...
class CompatibilityEngine:
    ...
    def analyze(self, bride, groom, precomputed_reports=None):
        ...
        divorce_risk = DivorceRiskEngine().evaluate(bride.get("chart", {}), groom.get("chart", {}))
        ...
        favorable_years = timing.get("favorable_years", ["2026", "2027"])
        best_muhurta = MarriageMuhurat().select(favorable_years, bride, groom)
        jaimini = JaiminiEngine().analyze_compatibility(bride, groom)
        ...
        final_score = (
            ...
            result["navamsha"].get("score", 0) * 0.10 +
            result["jaimini"].get("score", 0) * 0.10 +
            80 * 0.10 + 
