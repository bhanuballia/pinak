  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch Solar Return data');
        }
  ...
  return (
    <div className="min-h-screen bg-[#ffcccc] flex flex-col font-sans">
      ...
      <div className="bg-white border-b border-black flex justify-between items-center px-2 py-0.5 text-xs text-black shadow-sm font-sans">
        <span>{user_info}</span>
        <span>86. Monthly Solar Return 3 month overview</span>
      </div>
      ...
      <div className="bg-[#ffffcc] border-t border-black px-2 py-1 text-xs text-black font-sans mt-auto">
        This worksheet shows the Monthly progression chart and Local Monthly progression chart for last month, the current month and next month.
      </div>
    </div>
  );
};

export default SolarReturnViewer;
