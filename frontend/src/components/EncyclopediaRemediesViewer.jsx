...
export default function EncyclopediaRemediesViewer() {
  ...
  const [activeReportTopic, setActiveReportTopic] = useState(1);
  const totalReportTopics = 14;
  ...
  return (
    <div className="min-h-screen bg-rose-50 text-rose-950 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Header Banner */}
        <div className="bg-gradient-to-r from-rose-200 border border-rose-300 rounded-3xl p-6 md:p-8 shadow-xl relative overflow-hidden text-white">
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-4">
              <div className="bg-white/20 text-rose-600 p-3.5 rounded-2xl border border-white/30 backdrop-blur-sm">
                <BookOpen className="w-8 h-8" />
              </div>
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-amber-600 tracking-tight flex items-center gap-2">
                  Personalized Astrological Remedies
                </h1>
                <p className="text-xs md:text-[20px] text-rose-900 mt-1">
            ...
            {userAscendant && (
              <div className="hidden lg:flex flex-col items-end bg-white/10 border border-white/30 px-4 py-2 rounded-2xl backdrop-blur-sm">
                <span className="text-[20px] text-amber-600 font-semibold uppercase">Auto Detected Lagna</span>
                <span className="text-[22px] font-bold text-black">{userAscendant}</span>
              </div>
            )}
          </div>
        </div>
          ...
          return (
            <div className="grid grid-cols-1 gap-6">
              ...
              @media print {
                ...
                .print-report-container > div:first-child {
                  page-break-before: auto;
                  break-before: auto;
                }
              }
              @media screen {
                .print-report-container > div:not(:first-child) {
                  display: none;
                }
                .print-report-container > div:nth-child(${activeReportTopic + 1}) {
                  display: block;
                }
              }
            `}</style>
                  ...
                  <div className="print-report-container space-y-8 bg-white p-6 md:p-8 rounded-3xl border border-rose-200 shadow-md text-rose-950" style={{ fontSize: `${reportFontSize}px` }}>
                    {/* Header inside report (first child) */}
                    <div>
                      {/* COVER PAGE (Only visible on print) */}
                      <div className="hidden print:flex flex-col items-center justify-center min-h-[95vh] w-full text-center space-y-6">
                        <h1 className="text-5xl font-bold text-rose-950 uppercase tracking-widest border-b-2 border-rose-900 pb-6 mb-8">Astro Remedies Prescription</h1>
                        {(() => {
                          const bd = userChartData?.basic_details || userChartData?.basic || {};
                          const meta = userChartData?.meta || {};
                          const userName = bd.name || meta.name || 'Seeker';
                          const bDate = bd.birth_date || meta.date || meta.dob || 'Not Provided';
                          const bTime = bd.birth_time || meta.time || meta.tob || 'Not Provided';
                          const latStr = bd.lat || userChartData?.lat || 'N/A';
                          const lonStr = bd.lon || userChartData?.lon || 'N/A';
                          const locStr = bd.location || meta.location || `${latStr}, ${lonStr}`;

                          return (
                            <div className="text-2xl text-rose-900 space-y-5 font-medium mt-12 bg-rose-50 p-12 rounded-3xl border border-rose-200 shadow-sm w-4/5 max-w-3xl text-left">
                              <div className="flex justify-between border-b border-rose-200 pb-3">
                                <span className="text-rose-950 font-bold">Name:</span>
                                <span>{userName}</span>
                              </div>
                              <div className="flex justify-between border-b border-rose-200 pb-3">
                                <span className="text-rose-950 font-bold">Date of Birth:</span>
                                <span>{bDate}</span>
                              </div>
                              <div className="flex justify-between border-b border-rose-200 pb-3">
                                <span className="text-rose-950 font-bold">Time of Birth:</span>
                                <span>{bTime}</span>
                              </div>
                              <div className="flex justify-between border-b border-rose-200 pb-3">
                                <span className="text-rose-950 font-bold">Location:</span>
                                <span>{locStr}</span>
                              </div>
                              <div className="flex justify-between pt-2">
                                <span className="text-rose-950 font-bold">Lagna (Ascendant):</span>
                                <span className="text-amber-700 font-bold">{selectedLagna}</span>
                              </div>
                            </div>
                          );
                        })()}

                        <div className="mt-16 bg-white border border-rose-200 p-8 rounded-2xl w-4/5 max-w-3xl shadow-sm text-center">
                          <h3 className="text-2xl font-bold text-rose-950 mb-4 uppercase">Key Auspicious Gemstones</h3>
                          <div className="flex flex-col gap-3 text-xl font-mono text-rose-900">
                            <div><strong>Life Lord Gem:</strong> {lagnaInfo.lifeStone}</div>
                            <div><strong>Karaka Lord Gem:</strong> {lagnaInfo.karakaStone}</div>
                            <div><strong>Lucky Lord Gem:</strong> {lagnaInfo.luckyStone}</div>
                          </div>
                        </div>

                        <div className="mt-12 pt-12 text-xl text-rose-800 font-bold italic">
                          Generated on {new Date().toLocaleDateString()}
                        </div>
                      </div>

                      {/* STANDARD HEADER (Visible only on screen) */}
                      <div className="print:hidden text-center pb-6 border-b border-rose-200 space-y-2">
                    ...
                    {/* 1. Real-Time Transit-Based Remedies (Gochar Remedial Guide) */}
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <Sparkles className="w-5 h-5 text-amber-900" /> 1. Real-Time Transit-Based Remedies (Gochar Remedial Guide)
                      </h3>
                    ...
                    <div className="space-y-4">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <Gem className="w-5 h-5 text-amber-900" /> 2. Prescribed Auspicious Gemstones (Ratna)
                      </h3>
                      <p className="text-[18px] text-slate-900">
                    ...
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <BookOpen className="w-5 h-5 text-amber-900" /> 3. Beneficial Deities & Vedic Invocation Mantras
                      </h3>
                      <p className="text-[18px] text-slate-900">
                    ...
                    {/* 4. Recommended Rudrakshas */}
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <Layers className="w-5 h-5 text-amber-900" /> 4. Recommended Rudraksha Beads
                      </h3>
                      <p className="text-[18px] text-slate-900">
                    ...
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <Leaf className="w-5 h-5 text-amber-900" /> 5. Navagraha Plant Remedies
                      </h3>
                      <p className="text-[18px] text-slate-900">
                    ...
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <Calendar className="w-5 h-5 text-amber-900" /> 6. Planetary Relief Remedies
                      </h3>
                      <p className="text-[18px] text-slate-900">
                    ...
                    {/* 7. Color Therapy & Clothing Guide */}
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <Palette className="w-5 h-5 text-amber-900" /> 7. Color Therapy & Lifestyle Guide
                      </h3>
                      <p className="text-[18px] text-slate-900">
                    ...
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <Compass className="w-5 h-5 text-amber-900" /> 8. Prescribed Yantras & Sacred Geometries
                      </h3>
                      <p className="text-[18px] text-slate-900">
                    ...
                    {/* 9. Zodiac Sign (Lagna) Specific Remedies */}
                    ...
                    {/* 10. Lal Kitab House Remedies, Debts & Prohibitions */}
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-rose-950 flex items-center gap-2 border-b border-rose-200 pb-2">
                        <Heart className="w-5 h-5 text-rose-700" /> 10. Lal Kitab House Remedies, Debts & Warnings
                      </h3>
                      ...
                      return (
                        <div className="space-y-4 pt-2">
                          <h3 className="text-[18px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                            <Wind className="w-5 h-5 text-amber-900" /> 11. Vastu, Fengshui & Pyramid Recommendations (Element: {lagnaElement})
                          </h3>
                          <p className="text-[18px] text-slate-900">
                    ...
                    {/* 12. Prescribed Crystals, Lockets & Rosaries */}
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <Sparkles className="w-5 h-5 text-amber-900" /> 12. Prescribed Crystals, Lockets & Rosaries (Mala)
                      </h3>
                      <p className="text-[18px] text-slate-900">
                    ...
                    {/* 13. Sacred Mantra Sadhana & Code of Conduct */}
                    <div className="space-y-4 pt-2">
                      <h3 className="text-[20px] font-bold text-amber-900 flex items-center gap-2 border-b border-slate-800 pb-2">
                        <BookOpen className="w-5 h-5 text-amber-900" /> 13. Sacred Mantra Sadhana & Code of Conduct
                      </h3>
                      <p className="text-[18px] text-slate-900">
