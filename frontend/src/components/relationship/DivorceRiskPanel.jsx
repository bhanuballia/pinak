...
const DivorceRiskPanel = ({ report }) => {
  ...
  return (
    <div className="bg-slate-900 p-6 rounded-[2rem] border border-slate-700/50 backdrop-blur-xl relative overflow-hidden flex flex-col justify-center">
      ...
      <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-6">Separation Probability</h4>
      
      <div className="flex items-end justify-between mb-2">
        <div className="text-4xl font-serif italic font-bold text-white">
          {score}<span className="text-xl text-slate-500 not-italic">%</span>
        </div>
        <div className={`text-xs font-black tracking-widest ${isHighRisk ? 'text-rose-400' : 'text-emerald-400'}`}>
