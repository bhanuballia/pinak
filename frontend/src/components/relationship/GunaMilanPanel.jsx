...
const GunaMilanPanel = ({ report }) => {
  ...
  return (
    <div className="bg-orange-900/20 p-6 rounded-[2rem] border border-orange-500/20 backdrop-blur-md relative overflow-hidden flex flex-col justify-between">
      <div className="absolute top-0 left-0 w-32 h-32 bg-orange-500/10 blur-3xl rounded-full -ml-16 -mt-16"></div>
      
      <div className="flex justify-between items-start mb-6 relative z-10">
        <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-300">Ashtakoota Score</h4>
        <div className="text-orange-400 font-serif italic text-sm">Guna Milan</div>
      </div>

      <div className="relative z-10 text-center mb-6">
        <div className="flex items-end justify-center space-x-2">
          <span className="text-5xl font-serif italic text-white">{totalScore.toFixed(1)}</span>
          <span className="text-sm text-slate-400 pb-2 uppercase tracking-widest">/ 36</span>
        </div>
        <p className="text-xs text-orange-300/80 mt-2 font-medium tracking-wider">{interpretation}</p>
      </div>
