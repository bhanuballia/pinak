...
var addQueryString = (url, params) => {
  if (params && typeof params === "object") {
    ...
    for (const paramName of Object.keys(params)) {
      if (UNSAFE_KEYS.indexOf(paramName) > -1)
        continue;
...
var Backend = class {
  ...
  loadUrl(url, callback, languages, namespaces) {
    ...
    this.options.request(this.options, url, payload, (err, res) => {
      if (res && (res.status >= 500 && res.status < 600 || !res.status))
        return callback("failed loading " + safeUrl + "; status code: " + res.status, true);
      if (res && res.status >= 400 && res.status < 500)
        return callback("failed loading " + safeUrl + "; status code: " + res.status, false);
  ...
  create(languages, namespace, key, fallbackValue, callback) {
    ...
    languages.forEach((lng) => {
      ...
      if (url == null) {
        finished += 1;
        if (callback && finished === languages.length)
