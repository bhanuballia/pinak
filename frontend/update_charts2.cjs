...
chartsToUpdate.forEach(house => {
  const housePattern = house.replace(/\?/g, '\\?').replace(/\./g, '\\.');
  // Using [\s\S] to match across newlines within the tag
  const regex = new RegExp(`(<ZodiacChart[\\s\\S]*?houses={${housePattern}}[\\s\\S]*?)variant="legacy"`, 'g');
